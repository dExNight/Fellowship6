...
contract Pair is ERC20 {
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        uint balance0 = IERC20(token0).balanceOf(address(this)); // 500
        uint balance1 = IERC20(token1).balanceOf(address(this)); // 500
        uint amount0 = balance0.sub(_reserve0); // 500
        uint amount1 = balance1.sub(_reserve1); // 500

        bool feeOn = _mintFee(_reserve0, _reserve1); // false
        uint _totalSupply = totalSupply; // 500 // gas savings, must be defined here since totalSupply can update in _mintFee
        if (_totalSupply == 0) {
